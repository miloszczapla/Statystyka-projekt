...
```{r}
ramka_rok_do_roku_zmienne <- read_csv2("parsed_dane/ramka_rok_do_roku_zmienne.csv")
ceny_mil_euro <- read_csv2("parsed_dane/ceny_mil_euro.csv")
...
``` {r}
glimpse(ramka_rok_do_roku_zmienne)
```
...
```{r}
 ggplot(ramka_rok_do_roku_zmienne, aes(x=okres_czasu,y = cena_gazu,group = zakres_geograficzny,color = zakres_geograficzny)) +
  geom_line() +
  scale_y_continuous(breaks = seq(0.01, 0.08, by = 0.005)) +
...
geom_line() + 
  scale_y_continuous(breaks = seq(0.07, 0.21, by = 0.01)) +
...
# Koszty na jednego mieszkańca danego kraju mieszkanca w Euro

## koszty według roku w postaci tabelki
```{r}
koszty_mieszkania <- as.data.frame(ceny_mil_euro) %>%
  arrange(okres_czasu)
kable(koszty_mieszkania[1:10, c('zakres_geograficzny','okres_czasu','Food and non-alcoholic beverages','Alcoholic beverages, tobacco and narcotics','Health','Transport')]) #tylko pierwszych 10 rekordówz
